...
class ReleaseMetadataTests(unittest.TestCase):
    ...
    def test_current_release_metadata_passes(self):
        ...
        self.assertIn("RELEASE METADATA PASS (v1.7.4)", result.stdout)

    def test_historical_release_notes_are_extractable(self):
        for version in ("1.7.0", "1.7.1", "1.7.2", "1.7.3"):
    ...
    def test_version_mismatch_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("Current workflow version: **v1.7.4 baseline**.", text)
            readme.write_text(
                text.replace(
