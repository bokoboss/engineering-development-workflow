...
10. When coding-agent execution is required, translate relevant conclusions into the execution packet and provide: work mode + rationale, workspace write boundary, external-write approval status, required local workflow/skills, recommended model, reasoning effort, Ultra/multi-agent/parallel-mode decision, existing/new chat choice, explicit success gates, evidence that may be reused, cost/quota rationale for materially expensive settings, downgrade checkpoint for premium/high-effort work when practical, escalation trigger, and a ready-to-use bounded prompt according to `MODEL_ROUTING_POLICY.md`.
11. Treat model tier, reasoning effort, and execution parallelism as separate decisions. Use the least expensive combination likely to finish correctly. Do not escalate model tier and effort together without separate reasons, and keep Ultra/multi-agent premium modes off by default unless parallel exploration itself materially improves the task.
12. For premium/high-effort execution, consider an early checkpoint after diagnosis, architecture, migration planning, or the first bounded milestone. If Astra-level reasoning is resolved, step down first to GPT-6 Sol when stronger coding/agentic execution is still useful, or further to GPT-6 Luna (or GPT-5.6 Terra where current surface economics favor it) when the remaining work is mechanical or strongly testable. Reduce effort/parallelism as well when practical.
13. Decide whether risk justifies `independent-review`. For high-risk acceptance, do not rely solely on the executor's own narrative or self-review. A fresh context at the same model tier may be sufficient; use a stronger reviewer only when the review itself requires it.
14. After implementation, review the actual GitHub diff, tests, CI, artifacts, independent-review findings when required, and other required evidence before declaring the work accepted. Reuse valid revision-bound evidence instead of mechanically rerunning it.
15. Treat verification and review as finite and risk-driven. Classify findings as BLOCKER / REQUIRED / FOLLOW-UP. Once mandatory gates pass, required approvals/review are complete, and no BLOCKER/REQUIRED finding remains, close the task instead of continuing to search for optional polish or unrelated nearby defects. FAST/STANDARD remediation should follow the bounded review budget in `WORK_MODE_ROUTING.md`; repeated material failures trigger re-plan/escalation, not an open-ended inspect-fix-review loop.
16. For significant resolved defects/incidents, use the upstream postmortem skill when preserving the lesson is likely to prevent recurrence or reduce future diagnosis cost.
17. When work is recurring, event-driven, scheduled, or condition-watched, read upstream `CONTINUOUS_OPERATIONS.md`. Treat continuous operation as an optional outer layer around the core workflow, use `loop-readiness` before activation/autonomy increases, default new patterns to A1 observe/report, and preserve GitHub/project evidence as authoritative over operational memory.
